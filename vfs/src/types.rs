...
//
// No dependencies on anything else in this crate (or on `kernel`) — safe to
...
mod tests {
    ...
    #[test]
    fn stat_field_offsets_match_linux_x86_64_abi() {
        // Real `struct stat` (Linux x86-64): st_ino at 8, st_mode at 24,
        // st_size at 48. Verified by hand against the field list above
        // (st_dev:u64@0, st_ino:u64@8, st_nlink:u64@16, st_mode:u32@24,
        // st_uid:u32@28, st_gid:u32@32, _pad0:u32@36, st_rdev:u64@40,
        // st_size:i64@48), not assumed correct just because the struct
        // compiles to 144 bytes.
    ...
    #[test]
    fn file_type_dt_type_values() {
        assert_eq!(FileType::Regular.as_dt_type(), 8);
        assert_eq!(FileType::Directory.as_dt_type(), 4);
        assert_eq!(FileType::CharDevice.as_dt_type(), 2);
        assert_eq!(FileType::BlockDevice.as_dt_type(), 6);
        assert_eq!(FileType::Symlink.as_dt_type(), 10);
    }

    #[test]
    fn file_type_mode_bits_values() {
        assert_eq!(FileType::Regular.as_mode_bits(), 0o100000);
        assert_eq!(FileType::Directory.as_mode_bits(), 0o040000);
        assert_eq!(FileType::CharDevice.as_mode_bits(), 0o020000);
        assert_eq!(FileType::BlockDevice.as_mode_bits(), 0o060000);
        assert_eq!(FileType::Symlink.as_mode_bits(), 0o120000);
    }

    // ── Stat constructors ────────────────────────────────────────────────

    #[test]
    fn stat_dir_mode_and_nlink() {
        let s = Stat::dir(5);
        assert_eq!(s.st_ino, 5);
        assert_eq!(s.st_mode, FileType::Directory.as_mode_bits() | 0o755);
        assert_eq!(s.st_nlink, 2);
        assert_eq!(s.st_size, 0);
        assert_eq!(s.st_blocks, 0);
    }

    #[test]
    fn stat_regular_mode_and_nlink() {
        let s = Stat::regular(7, 100);
        assert_eq!(s.st_mode, FileType::Regular.as_mode_bits() | 0o444);
        assert_eq!(s.st_nlink, 1);
    }

    #[test]
    fn stat_regular_writable_mode() {
        let s = Stat::regular_writable(7, 100);
        assert_eq!(s.st_mode, FileType::Regular.as_mode_bits() | 0o644);
    }

    #[test]
    fn stat_executable_mode() {
        let s = Stat::executable(7, 100);
        assert_eq!(s.st_mode, FileType::Regular.as_mode_bits() | 0o555);
    }

    #[test]
    fn stat_symlink_mode_and_size() {
        let s = Stat::symlink(9, 42);
        assert_eq!(s.st_mode, FileType::Symlink.as_mode_bits() | 0o777);
        assert_eq!(s.st_size, 42);
        assert_eq!(s.st_nlink, 1);
        assert_eq!(s.st_blocks, 0);
    }

    #[test]
    fn stat_chardev_mode() {
        let s = Stat::chardev(3);
        assert_eq!(s.st_mode, FileType::CharDevice.as_mode_bits() | 0o666);
        assert_eq!(s.st_nlink, 1);
    }

    #[test]
    fn stat_with_perm_bits_keeps_type_replaces_low_12_bits() {
        let s = Stat::regular(1, 0).with_perm_bits(0o000);
        // Type bits survive, permission bits become exactly the low 12
        // bits passed in (0o000 here), regardless of the constructor's
        // own default permission bits (0o444 for `regular()`).
        assert_eq!(s.st_mode, FileType::Regular.as_mode_bits());

        let s = Stat::dir(1).with_perm_bits(0o1777);
        assert_eq!(s.st_mode, FileType::Directory.as_mode_bits() | 0o1777);

        // Passing bits outside the low 12 bits must not leak into the type
        // field.
        let s = Stat::regular(1, 0).with_perm_bits(0xFFFF_FFFF);
        assert_eq!(s.st_mode, FileType::Regular.as_mode_bits() | 0o7777);
    }

    #[test]
    fn stat_with_nlink_overrides_default() {
        let s = Stat::dir(1).with_nlink(9);
        assert_eq!(s.st_nlink, 9);
        let s = Stat::regular(1, 0).with_nlink(4);
        assert_eq!(s.st_nlink, 4);
    }

    #[test]
    fn stat_regular_blocks_rounds_up_to_512() {
        assert_eq!(Stat::regular(1, 0).st_blocks, 0);
        assert_eq!(Stat::regular(1, 1).st_blocks, 1);
        assert_eq!(Stat::regular(1, 511).st_blocks, 1);
        assert_eq!(Stat::regular(1, 512).st_blocks, 1);
        assert_eq!(Stat::regular(1, 513).st_blocks, 2);
    }

    // ── OpenFlags ────────────────────────────────────────────────────────

    #[test]
    fn open_flags_is_write() {
        assert!(!OpenFlags::RDONLY.is_write());
        assert!(OpenFlags::WRONLY.is_write());
        assert!(OpenFlags::RDWR.is_write());
        assert!(!OpenFlags(OpenFlags::CREAT.0).is_write());
    }

    #[test]
    fn open_flags_is_directory() {
        assert!(!OpenFlags::RDONLY.is_directory());
        assert!(OpenFlags::DIRECTORY.is_directory());
        assert!(OpenFlags(OpenFlags::RDONLY.0 | OpenFlags::DIRECTORY.0).is_directory());
    }

    // ── Errno ────────────────────────────────────────────────────────────

    #[test]
    fn errno_as_i64_is_negative() {
        assert_eq!(Errno::ENOENT.as_i64(), -2);
        assert_eq!(Errno::EIO.as_i64(), -5);
        assert_eq!(Errno::ELOOP.as_i64(), -40);
    }

    // ── DirEntry ─────────────────────────────────────────────────────────

    #[test]
    fn dir_entry_new_truncates_name_to_255_bytes() {
        let long_name = [b'a'; 300];
        let entry = DirEntry::new(1, FileType::Regular, &long_name);
        assert_eq!(entry.name_len, 255);
        assert!(entry.name[..255].iter().all(|&b| b == b'a'));
    }

    #[test]
    fn dir_entry_new_short_name_not_truncated() {
        let entry = DirEntry::new(1, FileType::Regular, b"hello");
        assert_eq!(entry.name_len, 5);
        assert_eq!(&entry.name[..5], b"hello");
    }

    #[test]
    fn dirent64_size_is_8_byte_aligned() {
        for name_len in [0usize, 1, 3, 4, 5, 7, 8, 12, 255] {
            let name = alloc::vec![b'x'; name_len];
            let entry = DirEntry::new(1, FileType::Regular, &name);
            let size = entry.dirent64_size();
            assert_eq!(size % 8, 0, "name_len={name_len} produced unaligned size {size}");
            // Must always be big enough for the fixed header (19 bytes)
            // plus the name plus a null terminator.
            assert!(size >= 19 + name_len + 1);
        }
    }

    #[test]
    fn write_dirent64_layout() {
        let entry = DirEntry::new(0x1234, FileType::Directory, b"foo");
        let size = entry.dirent64_size();
        let mut buf = alloc::vec![0xFFu8; size];
        entry.write_dirent64(0x5678, &mut buf);

        assert_eq!(u64::from_le_bytes(buf[0..8].try_into().unwrap()), 0x1234);
        assert_eq!(i64::from_le_bytes(buf[8..16].try_into().unwrap()), 0x5678);
        assert_eq!(u16::from_le_bytes(buf[16..18].try_into().unwrap()), size as u16);
        assert_eq!(buf[18], FileType::Directory.as_dt_type());
        assert_eq!(&buf[19..22], b"foo");
        assert_eq!(buf[22], 0); // null terminator
        // Padding from the terminator to reclen is zeroed, not left as the
        // 0xFF the buffer was pre-filled with.
        assert!(buf[23..size].iter().all(|&b| b == 0));
    }

    #[test]
    fn write_dirent64_empty_name() {
        let entry = DirEntry::new(1, FileType::Regular, b"");
        let size = entry.dirent64_size();
        let mut buf = alloc::vec![0xAAu8; size];
        entry.write_dirent64(8, &mut buf);
        assert_eq!(buf[19], 0); // just the null terminator
        assert!(buf[20..size].iter().all(|&b| b == 0));
    }
}
