...
smoke tests (`RUST_PROGRAMS`, all well under 50 KiB), and `kdebug` (the
...
`ext2_robust_test`, `fpu_test`, `socket_test`, `pipe_cow_test`, `sigsuspend_test`, `lifecycle_test`, `shm_test`, `pipe_multi_test`, `fb0_test`, `wait_intr_test`, `input_poll_test`, `session_test`) — is built straight to
