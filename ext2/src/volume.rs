...
//
// Directory operations (step 4) have since moved too, but into `dir.rs`
// (a separate `impl Ext2Core` block there) rather than this file — see
...
// step 5) — `mount` below only parses the superblock and constructs
// `Self`, matching what `Ext2Fs::mount()` used to do before its
// repair-pass calls were added (those live in
// `kernel::fs::ext2::mount_and_repair` now, unmigrated).
...
mod tests {
    use super::*;
    // `minimal_image`/`mount` used to be private copies inside this module
    // — pulled out to `crate::test_support` (migration step 4) so
    // `dir.rs`'s directory-operation tests can reuse the exact same fixture
    // instead of a second, possibly-drifting copy. See that module's doc
    // comment for why it's a deliberately different shape from the
    // kernel's own `build_minimal_image` (no pre-written root directory
    // data block).
