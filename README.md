...
- **Pipes** (`pipe(2)`): IPC anónima con ring buffer, lectura/escritura bloqueante, `EOF`/`EPIPE`, fds heredados por `fork()` (refcount por extremo vía `FileHandle::dup()`). Un `seq 1 3000 | wc -c` se colgaba para siempre: `sys_write` bloqueaba con `jump_to_user` (que nunca vuelve) con un `Arc` de la tabla de descriptores todavía vivo en la pila abandonada, así que la tabla —y el extremo de escritura— sobrevivía al proceso y el lector nunca veía `EOF`. Encontrado con gdb sobre el cuelgue vivo; la regla que deja es que nada con `Drop` puede quedar vivo antes de un `-> !`.
...
- **Sockets AF_UNIX reales**: `SOCK_STREAM`/`SOCK_DGRAM`, rutas en el filesystem y namespace abstracto, `listen`/`accept`, half-close, `SO_*` y paso de descriptores con `SCM_RIGHTS`, con `poll`/`epoll`. Las máquinas de estado viven en el crate `usock`, testeable en el host.
...
- **Teclado USB vía un driver xHCI propio** (`hal/src/{xhci,usb,hid}.rs` +
  ...
  existe ahí).
- **`/mnt` desde el propio pendrive, por USB Mass Storage** (`hal/src/{msc,gpt}.rs`,
  `kernel/src/usb/xhci/msc.rs`): la máquina no tiene IDE, así que el ext2 vive en una
  partición GPT (`constanos-data`) del pendrive de arranque. Encima, una **caché de
  bloques** (`hal::blockcache`, write-through, 32 MiB, lectura anticipada de 64 KiB):
  ext2 pedía un bloque de 1 KiB por comando SCSI y releía los bloques indirectos por
  cada bloque de datos, así que arrancar `doom` eran ~126.000 comandos; ahora son ~300,
  y un segundo `doom` no toca el pendrive.
- **El log del kernel en el pendrive**: el ring de `klog` se copia a una partición
  cruda (`constanos-log`) cada 5 s, con `sync` y desde el propio handler de pánico, un
  slot por arranque. `scripts/usb-log.sh read`, desde el Linux de la misma máquina,
  lo lee después de reiniciar. Así se diagnosticaron los tres bugs siguientes sin una
  sola foto de la pantalla.
- **IRQs espurias del 8259**: abrir `doom` paniqueaba de vez en cuando con un GPF de
  código `0x13B` — vector 39, la IRQ 7 espuria que el PIC entrega en hardware real y
  QEMU casi nunca. La IDT no tenía entrada ahí, y una entrada vacía no se ignora: es un
  `#GP`. Ahora los 16 vectores del PIC tienen manejador (con la comprobación del ISR
  para 7/15) y las líneas sin uso quedan enmascaradas.
- **El marco de interrupción se leía por el lado equivocado**: los manejadores
  declaraban `&mut ExceptionStackFrame`, pero el ABI `x86-interrupt` pasa el marco por
  valor, así que el RIP se usaba como puntero. El "RIP" de cada informe eran bytes de
  código, y la comprobación "¿vino de modo usuario?" leía código — un fault de un
  proceso podía tratarse como fault del kernel.
...
- **`/proc/fbinfo`**: el panel de instrumentos de la consola. Geometría real
  ...
  *non-posted*: el CPU se para hasta que el dato vuelve por el bus). Se arregló en
  cuatro fases, medidas en metal: un shadow buffer en RAM (la VRAM solo se escribe),
  `IA32_PAT` reprogramado con una entrada write-combining, el framebuffer mapeado WC,
  y un `blit_scaled` por scanlines. `seq 1 400` pasó de **875 s a 0,75 s**, y un frame
  de DOOM de 26 ms a 5,2 ms. Registro completo en `docs/fb/console-perf.md`.
...
| `quake` | **Quake real, jugable, con sonido** — [quakegeneric](https://github.com/erysdren/quakegeneric) + puerto propio sobre `FBIO_BLIT` (con conversión índice→RGB propia, el motor entrega paletizado), `/dev/input/event0`+`event1` (evdev real, pull-based), shareware `id1/pak0.pak` leído de `/mnt` (ext2), efectos de sonido reales vía `/dev/dsp`/AC97. Ver la entrada de arriba |
...
│   │   ├── ipc/          # Adaptador de sockets AF_UNIX (la lógica vive en usock/)
│   │   ├── block/        # ATA PIO, el pendrive USB como dispositivo de bloques, la partición de log
...
├── scripts/deploy-usb-boot.sh # Lleva el kernel a la partición `boot` del pendrive (probado en QEMU antes)
├── scripts/sync-usb-data.sh   # Sincroniza disk-image-root/ con la partición de datos del pendrive
├── scripts/usb-log.sh         # Lee el log del kernel guardado en el pendrive
...
(ext2, `/mnt`) y `constanos-log` (cruda, el log). Después de `cargo build`:

```bash
scripts/deploy-usb-boot.sh   # kernel → `boot` (arma la FAT, arranca en QEMU, escribe y verifica)
scripts/sync-usb-data.sh     # disk-image-root/ → `constanos-data`
# ...arrancar la máquina, usarla, `kdebug sync`, volver a Linux...
scripts/usb-log.sh read      # el log de ese arranque
```

Nunca hacer `dd` de la imagen entera al dispositivo: reemplaza la tabla GPT y se lleva
las otras dos particiones.
...
- ✅ **Consola lenta en metal, arreglado**: shadow buffer en RAM + write-combining vía `IA32_PAT` (ver la sección de hardware real). `seq 1 400`: 875 s → 0,75 s en la Ryzen.
